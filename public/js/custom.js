...
var do_on_load = function() {
    ...
    $('.home-autocomplete').each(function(){
        var input = $(this);
        input.autocomplete(input.attr('data-autocomplete-url'),{
            matchContains:1, //also match inside of strings when caching
            mustMatch:1, //allow only values from the list
...
function addedWho(){
    ...
    $('input:checkbox.person').each(function () {
        ...
        if ($(this).is(':checked'))
        {
            ...
            {
                $('#who-view').append("<li id="+personspanid+"><span class='name'>"+person_name+"</span>"+
                    "<span id="+person_price+" class='price item-price-summary'>$"+ parseFloat(person_price).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,')+
                    "</span></li>");
            }
...
{
    $('#what-view').append("<li id="+spanid+"><span class='name'>"+option_text+"</span><span id="+span_price+" class='price item-price-summary'>$"+parseFloat(new_option_price).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,')+"</span></li>");
}
...
function add_person_fields(link, association, content,person_content,defaultAction) {
   ...
   // console.log();
    addPersonAddon(person_id,person_name,association,content,defaultAction);


    $(".person-list").append(person_content.replace(/{demo_id}/g,person_id).replace(/{demo_name}/g,person_name).replace(/_demo_id_/g,person_id).replace(/_demo_name/g,person_name));
    $('[data-for-person-id="' + person_id + '"]').find(".price-option").each(function(){
...
function addPersonAddon(person_id,person_name,association,content,defaultAction){
    var new_id = new Date().getTime();
   ...
   // $(".addons").append(content.replace(regexp, new_id));
    ...
    var current_person_element  = $("ul").find("[data-person='" + person_name + "']");

    current_person_element.append(content.replace(regexp, new_id));

    var options_person = current_person_element.find('.options-person');
    ...
    options_person.find('.rule').first().remove();
}
...
function personCartToggle(el,id,person_name,association,content) {
    ...
    if(el.checked){ //If the user CHECKED the box ADD the person to the cart
        ...
        addPersonAddon(id,person_name,association,content,1);
...
function addGuest( association, content,person_content){
    ...
    console.log(jQuery(document).data('tcEvents.guests'));
    ...
    var new_content = content.replace(regexp, new_id).replace(/{guest_id}/g,guest_id).replace(/{guest_name}/g,guest_name).replace(/guest_id/g,guest_id);
    ...
    var options = guestEl.find('.options');

    $(new_content).insertAfter(options);
   // console.log(guestEl.find('.options'));
    ...
    guestEl.find('.rule').first().remove();
